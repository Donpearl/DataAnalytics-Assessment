--  1: Compute monthly successful transactions per customer
WITH monthly_transactions AS (
    SELECT 
        a.owner_id,  -- identifier for the customer
        DATE_FORMAT(a.transaction_date, '%Y-%m') AS month_year,  -- Format date to Year-Month
        COUNT(*) AS transaction_count  -- Number of successful transactions in that month
    FROM 
        savings_savingsaccount a
    WHERE 
        a.transaction_status = 'successful'  -- Include only successful transactions
        AND a.transaction_date IS NOT NULL   -- Exclude records without a transaction date
    GROUP BY 
        a.owner_id, month_year
),

-- 2: Calculate average monthly transaction count per customer
...
--  3: Categorize customers based on the average monthly transactions
SELECT 
    CASE 
